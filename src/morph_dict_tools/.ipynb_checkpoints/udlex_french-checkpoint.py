...
def prepareFrenchDict(upos_filter=None, verbose=False):
    ...
    apertium = pd.read_csv(fr1, names=cols, delimiter="\t", index_col=False, on_bad_lines='skip')
    ...
    with open(fr2+"prepr", 'w') as f:
        f.write('\n'.join(lines))
    lefff = pd.read_csv(fr2+"prepr", names=cols, delimiter="\t", index_col=False, on_bad_lines='skip')
